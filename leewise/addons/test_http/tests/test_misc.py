...
import leewise
from leewise.http import root, content_disposition
from leewise.tests import tagged
from leewise.tests.common import HOST, new_test_user, get_db_name, BaseCase
from leewise.tools import config, file_path
from leewise.addons.test_http.controllers import CT_JSON

from leewise.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = leewise.tools.misc.DotDict(db=False)
            return leewise.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'leewise.in/{uri}', host='leewise.in'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://leewise.in/{uri}', host='leewise.in'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'leewise.in/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://leewise.in/{uri}'), "No host allowed")
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'leewise.in',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict('leewise.tools.config.options', {'proxy_mode': True}):
    ...
    def test_misc6_upload_file_retry(self):
        from leewise.addons.test_http import controllers  # pylint: disable=C0415
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(leewise.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db2_use_session_db(self):
        ...
        session.db = 'db0'
        leewise.http.root.session_store.save(session)
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = leewise.http.root.session_store.get(res.cookies['session_id'])
