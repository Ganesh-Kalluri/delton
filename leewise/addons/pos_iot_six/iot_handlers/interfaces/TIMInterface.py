...
from leewise.addons.hw_drivers.tools import helpers
from leewise.tools.misc import file_path

_logger = logging.getLogger(__name__)

if system() == 'Windows':
    LIB_PATH = Path('leewise/addons/hw_drivers/iot_handlers/lib')
    LIB_EXTENSION = '_w.dll'
    IMPORT_LIBRARY = ctypes.WinDLL
    DOWNLOAD_URL = 'https://nightly.leewise.in/master/posbox/iotbox/six-timapiv23_09_w.zip'
else:
    LIB_PATH = file_path('hw_drivers/iot_handlers/lib')
    LIB_EXTENSION = '_l.so'
    IMPORT_LIBRARY = ctypes.CDLL
    DOWNLOAD_URL = 'https://nightly.leewise.in/master/posbox/iotbox/six-timapiv23_09_l.zip'
...
# Import Leewise Timapi Library
TIMAPI_LIB_PATH = f'{LIB_PATH}/tim/libsix_leewise{LIB_EXTENSION}'
try:
...
class TIMInterface(Interface):
    ...
    def get_devices(self):
        ...
        new_tid = helpers.read_file_first_line('leewise-six-payment-terminal.conf')
        devices = {}
