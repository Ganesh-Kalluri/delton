...
import leewise
from leewise.tools.misc import mute_logger
from leewise.addons.mail.models.partner_devices import InvalidVapidError
from leewise.addons.mail.tests.common import mail_new_test_user
from leewise.addons.sms.tests.common import SMSCommon
...
class TestWebPushNotification(SMSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env['mail.partner.device'].sudo().create([{
            'endpoint': 'https://test.leewise.in/webpush/user1',
        ...
        cls.env['mail.partner.device'].sudo().create([{
            'endpoint': 'https://test.leewise.in/webpush/user2',
            'expiration_time': None,
            'keys': json.dumps({
                'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                'auth': 'DJFdtAgZwrT6yYkUMgUqow'
    ...
    @patch.object(leewise.addons.mail.models.mail_thread, 'push_to_end_point')
    def test_push_notifications(self, push_to_end_point):
        ...
        self.assertEqual(push_to_end_point.call_args.kwargs['device']['endpoint'], 'https://test.leewise.in/webpush/user2')
    ...
    @patch.object(leewise.addons.mail.models.web_push, 'push_to_end_point')
    def test_push_notifications_cron(self, push_to_end_point):
        ...
        for index in range(10, 14):
            self.env['mail.partner.device'].sudo().create([{
                'endpoint': 'https://test.leewise.in/webpush/user%d' % index,
                'expiration_time': None,
                'keys': json.dumps({
                    'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                    'auth': 'DJFdtAgZwrT6yYkUMgUqow'
    ...
    @patch.object(leewise.addons.mail.models.mail_thread.Session, 'post',
    ...
    def test_push_notifications_encryption_simple(self, post):
        ...
        self.assertEqual(post.call_args.args[0], 'https://test.leewise.in/webpush/user2')
    ...
    def test_push_notifications_device_unreachable(self, post):
        with mute_logger('leewise.addons.mail.web_push'):
            self.record_simple.with_user(self.user_email).message_notify(
                partner_ids=self.user_inbox.partner_id.ids,
                body='Test message send via Web Push',
                subject='Test Activity',
        ...
        notification_count = self.env['mail.partner.device'].search_count([('endpoint', '=', 'https://test.leewise.in/webpush/user2')])
