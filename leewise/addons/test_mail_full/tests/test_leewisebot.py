...
from leewise.addons.mail.tests.common import MailCommon
from leewise.addons.test_mail.tests.common import TestRecipients
from leewise.tests import tagged
from leewise.tools import mute_logger


@tagged("leewisebot")
class TestLeewisebot(MailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestLeewisebot, cls).setUpClass()
        cls.test_record = cls.env['mail.test.simple'].with_context(cls._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        cls.leewisebot = cls.env.ref("base.partner_root")
        ...
        cls.leewisebot_ping_body = '<a href="http://leewise.in/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@LeewiseBot</a>' % (cls.leewisebot.id, cls.leewisebot.id)
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('leewise.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_leewisebot()
        leewisebot = self.env.ref("base.partner_root")
        leewisebot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', leewisebot.id)])
        self.assertEqual(len(leewisebot_in_fetch_listeners), 1, 'leewisebot should appear only once in channel_fetch_listeners')

    @mute_logger('leewise.addons.mail.models.mail_mail')
    def test_leewisebot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.leewisebot_ping_body, 'partner_ids': [self.leewisebot.id, self.user_admin.partner_id.id]})
        ...
        with patch('random.choice', lambda x: x[0]):
            self.assertNextMessage(
                ...
                sender=self.leewisebot,
                answer=False
            )
        # Leewisebot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.leewisebot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last leewisebot message instead
            sender=self.leewisebot,
            answer=("@LeewiseBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.leewisebot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.leewisebot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('leewise.addons.mail.models.mail_mail')
    def test_leewisebot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @leewisebot right now", 'partner_ids': []})
        self.assertNextMessage(
