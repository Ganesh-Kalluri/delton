...
from leewise.addons.crm.tests.common import TestCrmCommon
from leewise.addons.iap.tools import iap_tools
...
class TestCRMLead(TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.emails_provider_generic = {
            ...
            ('fp@leewise.in', 'fp@leewise.in'),
            ('fp.alias@mail.leewise.in', 'fp.alias@mail.leewise.in'),
        }
        cls.emails_provider_company = {
            ('robert.poilvert@mycompany.com', 'mycompany.com'),
            ('fp@subdomain.leewise.in', 'subdomain.leewise.in'),
        }
        ...
        cls.lead_generic_email_dupes = cls.env['crm.lead'].create([
            ...
            {
                'email_from': '"Fabulous Fab" <fp@leewise.in>',
                'name': 'Dupe1 of fp@leewise.in (same email)',
                'type': 'lead',
            },
            {
                'email_from': 'FP@leewise.in',
                'name': 'Dupe2 of fp@leewise.in (same email)',
                'type': 'lead',
            },
            # phone_sanitized based
            {
                'email_from': 'not.fp@not.leewise.in',
                'name': 'Dupe3 of fp@leewise.in (same phone sanitized)',
                'phone': '+1 202 555 0123',
                'type': 'lead',
            },
            {
                ...
                'name': 'Dupe4 of fp@leewise.in (same phone sanitized)',
                'type': 'lead',
            },
            # same commercial entity
            {
                'name': 'Dupe5 of fp@leewise.in (same commercial entity)',
                'partner_id': cls.test_partners[1].id,
            },
            {
                'name': 'Dupe6 of fp@leewise.in (same commercial entity)',
    ...
    def test_assert_initial_values(self):
        ...
        self.assertEqual(lead_generic.email_domain_criterion, 'fp@leewise.in')
        self.assertEqual(lead_generic.email_normalized, 'fp@leewise.in')
