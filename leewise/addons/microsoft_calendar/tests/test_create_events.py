...
from leewise.addons.microsoft_calendar.utils.microsoft_calendar import MicrosoftCalendarService
from leewise.addons.microsoft_calendar.utils.microsoft_event import MicrosoftEvent
from leewise.addons.microsoft_calendar.models.res_users import User
from leewise.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A Leewise event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_simple_event_without_email(self):
        ...
        with self.assertRaises(ValidationError):
            record._sync_leewise2microsoft()

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Leewise organizer calendar.
        """
        ...
        self.assert_leewise_event(new_records, self.expected_leewise_event_from_outlook)
        self.assertEqual(new_records.user_id, self.organizer_user)
        self.assertEqual(new_records.need_sync_m, False)

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_exists_in_leewise(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Leewise attendee calendar.
        There is a Leewise user that matches with the organizer email address.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_leewise(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Leewise attendee calendar.
        no Leewise user that matches with the organizer email address.
        """
        ...
        outlook_event = dict(self.simple_event_from_outlook_attendee, organizer={
            'emailAddress': {'address': "john.doe@leewise.in", 'name': "John Doe"},
        })
        expected_event = dict(self.expected_leewise_event_from_outlook, user_id=False)
        ...
        new_records = (records - existing_records)
        self.assertEqual(len(new_records), 1)
        self.assert_leewise_event(new_records, expected_event)

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_where_email_addresses_are_capitalized(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Leewise attendee calendar.
        The email addresses of the attendee and the organizer are in different case than in Leewise.
        """
    ...
    def test_create_recurrent_event_without_sync(self, mock_insert):
        """
        A Leewise recurrent event is created when Outlook sync is not enabled.
        """
        if not self.sync_leewise_recurrences_with_outlook_feature():
            return
    ...
    def test_create_recurrent_event_with_sync(self, mock_insert, mock_get_events):
        """
        A Leewise recurrent event is created when Outlook sync is enabled.
        """
        if not self.sync_leewise_recurrences_with_outlook_feature():
            return
    ...
    def test_create_recurrent_event_with_sync_by_another_user(self, mock_insert, mock_get_events):
        """
        A Leewise recurrent event has been created and synced with Outlook by another user, but nothing
    ...
    def test_create_recurrent_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Leewise organizer calendar.
        """
        ...
        self.assert_leewise_recurrence(new_recurrences, self.expected_leewise_recurrency_from_outlook)
        for i, e in enumerate(sorted(new_events, key=lambda e: e.id)):
            self.assert_leewise_event(e, self.expected_leewise_recurrency_events_from_outlook[i])

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_recurrent_event_from_outlook_attendee_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Leewise attendee calendar.
        """
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in Leewise due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in Leewise Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its Leewise Calendar synced.
        ...
        # In Leewise, this event will be created (behind the screens) by a synced Leewise user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with Leewise through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in Leewise (no ACL error was triggered blocking creation).
    ...
    def test_create_simple_event_from_outlook_without_organizer(self, mock_get_events):
        """
        Allow creation of an event without organizer in Outlook and sync it in Leewise.
        """
