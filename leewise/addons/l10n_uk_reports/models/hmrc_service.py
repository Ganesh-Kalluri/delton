...
if DEBUG:
    ...
    PROXY_SERVER = 'https://l10n-uk-hmrc.test.leewise.in'
else:
...
class HmrcService(models.AbstractModel):
    ...
    def _get_fraud_prevention_info(self, client_data=None):
        ...
        try:
            ...
            gov_dict['Gov-Vendor-Version'] = '&'.join([urls.url_quote("Leewise") + "=" + urls.url_quote(gov_vendor_version)]*2) # We can not percent encode the separator and we need to do it for the key and the value. Client and Server sides are the same
            gov_dict['Gov-Client-User-IDs'] = urls.url_quote("Leewise") + "=" + urls.url_quote(self.env.user.name)  # We can not percent encode the separator, same as Gov-Vendor-Version
            ...
            gov_dict['Gov-Vendor-Product-Name'] = urls.url_quote("Leewise")
            ...
            if hashed_license:
                gov_dict['Gov-Vendor-License-IDs'] = urls.url_quote("Leewise") + "=" + urls.url_quote(hashed_license)
        except Exception:
