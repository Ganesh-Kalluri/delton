...
QUnit.test("chat window should not open when receiving a new DM from leewisebot", async () => {
    const pyEnv = await startServer();
    const userId = pyEnv["res.users"].create({ partner_id: pyEnv.leewisebotId });
    const channelId = pyEnv["discuss.channel"].create({
        channel_member_ids: [
            Command.create({ is_pinned: false, partner_id: pyEnv.currentPartnerId }),
            Command.create({ partner_id: pyEnv.leewisebotId }),
        ],
        channel_type: "chat",
    });
    const { env } = await start();
