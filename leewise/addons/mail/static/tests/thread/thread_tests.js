...
QUnit.test(
    ...
    async () => {
        ...
        const { openDiscuss } = await start();
        openDiscuss(channelId);
        await insertText(".o-mail-Composer-input", "email@leewise.in\n@Te");
        await click(".o-mail-Composer-suggestion");
        await contains(".o-mail-Composer-input", { value: "email@leewise.in\n@TestPartner " });
        await click(".o-mail-Composer-send:enabled");
        await contains(
            `.o-mail-Message-body .o_mail_redirect[data-oe-id="${partnerId}"][data-oe-model="res.partner"]`,
            { text: "@TestPartner" }
...
QUnit.test("Transient messages are added at the end of the thread", async () => {
    ...
    await contains(":nth-child(2 of .o-mail-Message)", { text: "LeewiseBot" });
});
