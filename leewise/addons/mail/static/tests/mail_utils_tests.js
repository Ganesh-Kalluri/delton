...
QUnit.test("add_link utility function", function (assert) {
    const testInputs = {
        ...
        "https://www.transifex.com/leewise/leewise-11/translate/#fr/lunch?q=text%3A'La+Tartiflette'": true,
        "https://www.transifex.com/leewise/leewise-11/translate/#fr/$/119303430?q=text%3ATartiflette": true,
...
QUnit.test("addLink: utility function and special entities", function (assert) {
    const testInputs = {
        ...
        "https://leewise.in/%5B%5D": `<a target="_blank" rel="noreferrer noopener" href="https://leewise.in/%5B%5D">https://leewise.in/[]</a>`,
    };
...
QUnit.test("url", async () => {
    ...
    const messageBody = "https://leewise.in?test=~^|`{}[]#";
...
QUnit.test("url with comma at the end", async () => {
    ...
    const { openDiscuss } = await start();
    await openDiscuss(channelId);
    const messageBody = "Go to https://leewise.in, it's great!";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button:enabled", { text: "Send" });
    await contains(".o-mail-Message a", { text: "https://leewise.in" });
...
QUnit.test("url with number in subdomain", async () => {
    ...
    const { openDiscuss } = await start();
    await openDiscuss(channelId);
    const messageBody = "https://www.45017478-master-all.runbot134.leewise.in/web";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button:enabled", { text: "Send" });
    await contains(".o-mail-Message a", {
        text: "https://www.45017478-master-all.runbot134.leewise.in/web",
    });
});
