...
from leewise.tests.common import new_test_user
from leewise.exceptions import ValidationError
from leewise.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api
from leewise.addons.google_calendar.utils.google_calendar import GoogleEvent
from leewise import Command, tools

class TestSyncGoogle2Leewise(TestSyncGoogle):
    ...
    def sync(self, events):
        ...
        self.env['calendar.recurrence']._sync_google2leewise(google_recurrence)
        self.env['calendar.event']._sync_google2leewise(events - google_recurrence)
    ...
    def test_new_google_event(self):
        ...
        values = {
            ...
            'organizer': {'email': 'leewisecalendarref@gmail.com', 'self': True},
        ...
        self.env['calendar.event']._sync_google2leewise(GoogleEvent([values]))
    ...
    def test_attendee_cancelled(self):
        ...
        self.env['calendar.event'].with_user(user)._sync_google2leewise(gevent)
    ...
    def test_private_extended_properties(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'leewisebot@example.com', 'responseStatus': 'declined'}],
            'extendedProperties': {'private': {'%s_leewise_id' % self.env.cr.dbname: event.id}},
    ...
    def test_recurrence(self):
        ...
        self.env['calendar.recurrence']._sync_google2leewise(GoogleEvent([values]))
    ...
    def test_recurrence_moved(self):
        ...
        values = {
            ...
            "attendees": [
                {
                    "email": "leewisebot@example.com", "responseStatus": "accepted",
                },
            ],
            'updated': self.now,
            'guestsCanModify': True,
    ...
    def test_recurrence_no_duplicate(self):
        ...
        self.env['calendar.recurrence']._sync_google2leewise(google_events)
    ...
    def test_simple_event_into_recurrency(self):
        ...
        recurrence = self.env['calendar.recurrence']._sync_google2leewise(GoogleEvent([values]))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
    ...
    def test_attendees_same_event_both_share(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'c.c@example.com', 'responseStatus': 'needsAction'},
                          {'email': 'leewisebot@example.com', 'responseStatus': 'accepted'},],
            'extendedProperties': {'shared': {'%s_leewise_id' % self.env.cr.dbname: event.id,
    ...
    def test_several_attendee_have_the_same_mail(self):
        """
        In google, One mail = One attendee but on Leewise, some partners could share the same mail
        ...
        self.assertEqual(mails, ['dalton@example.com', 'leewisebot@example.com'])
        gevent = GoogleEvent([{
            'id': google_id,
            'description': 'coucou',
            "updated": self.now,
            'organizer': {'email': 'leewisebot@example.com', 'self': True},
            'summary': False,
            'visibility': 'public',
            'attendees': [],
            'reminders': {'useDefault': True},
    ...
    def test_several_users_have_the_same_mail(self):
        ...
        event = self.env['calendar.event']._sync_google2leewise(GoogleEvent([values]))
    ...
    def test_event_with_meeting_url(self):
        values = {
            ...
            'conferenceData': {
                'entryPoints': [{
                    ...
                    'uri': 'https://meet.google.com/leewise-random-test',
                    'label': 'meet.google.com/leewise-random-test'
                }, {
                    'entryPointType': 'more',
                    'uri':'https://tel.meet/leewise-random-test?pin=42424242424242',
                    'pin':'42424242424242'
                }]
            }
        }
        ...
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
        self.assertTrue(event, "It should have created an event")
        self.assertEqual(event.videocall_location, 'https://meet.google.com/leewise-random-test')
        self.assertGoogleAPINotCalled()
    ...
    def test_partner_order(self):
        self.private_partner.email = "internal_user@leewise.in"
        ...
        values = {
            'id': 'oj44nep1ldf8a3ll02uip0c9aa',
            'description': 'Small mini desc',
            'organizer': {'email': 'internal_user@leewise.in'},
            'summary': 'Pricing new update',
            'visibility': 'public',
            'attendees': [{
                'displayName': 'Mitchell Admin',
        ...
        self.env['calendar.event'].with_user(user)._sync_google2leewise(GoogleEvent([values]))
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
    ...
    def test_recurrence_range_start_date_in_other_dst_period(self):
        ...
        self.env['calendar.recurrence']._sync_google2leewise(google_event)
    ...
    def test_recurrence_edit_specific_event(self):
        ...
        self.env['calendar.recurrence']._sync_google2leewise(recurrent_events)
        ...
        self.env['calendar.event']._sync_google2leewise(specific_event)
        events = self.env["calendar.event"].search(
            [
                ("name", "=", specific_event.summary),
                ("start", ">=", day_start),
    ...
    def test_event_guest_modify_permission(self):
        ...
        This test states that Leewise Calendar:
        ...
        # Sync events from Google to Leewise and get them after sync.
        self.env['calendar.event']._sync_google2leewise(GoogleEvent([not_editable_event_values, editable_event_values]))
        ...
        # Assert that event is created in Leewise with proper values for guests_readonly variable.
    ...
    def test_attendee_status_is_not_updated_when_syncing_and_time_data_is_not_changed(self):
        ...
        self.env['calendar.recurrence'].with_user(other_user)._sync_google2leewise(GoogleEvent(values))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
