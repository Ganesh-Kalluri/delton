...
from leewise import api, fields, models, _lt
from leewise.addons.iap.tools.iap_tools import iap_jsonrpc
from leewise.exceptions import AccessError
from urllib.parse import urljoin, urlparse

DEFAULT_WSS_ENDPOINT = 'https://iap-scraper.leewise.in/'
...
class WebsiteGeneratorRequest(models.Model):
    ...
    def create(self, vals_list):
        ...
        for req in wg_requests:
            ...
            # created via the leewise.in start trial.
            if not req.uuid:
    ...
    def _generate_site(self, tar):
        leewise_blocks = self._load_input(tar)
        website = self._get_website(leewise_blocks, tar)
        # Generate the images attachments (Modifies leewise_blocks in place)
        self._save_images_as_attachments(leewise_blocks, tar)
        self._generate_pages(website, leewise_blocks)
        return website, leewise_blocks
    ...
    def _generate_pages(self, website, leewise_blocks):
        # Create pages
        for page_url, page_data in leewise_blocks.get('pages', {}).items():
        ...
        homepage._construct_homepage(leewise_blocks['homepage'])

    def _get_website(self, leewise_blocks, tar):
        website = self.env['website'].get_current_website()
        website_info = leewise_blocks.get('website')
        if not website_info:
            raise ValueError("Website info not found in the input")
        homepage_url = leewise_blocks.get('homepage', {}).get('url')
        if not homepage_url:
    ...
    def _save_images_as_attachments(self, leewise_blocks, tar):
        all_images = leewise_blocks['website'].get('all_images', {})
        ...
        # Home page
        for image in leewise_blocks['homepage'].get('images_to_crop', []):
        ...
        # Other pages
        for page_dict in leewise_blocks.get('pages', {}).values():
        ...
        if attachments_url_src:
            # Modifies leewise_blocks in place
            self._update_html_urls(leewise_blocks, attachments_url_src, cropped_attachments_url_src)
    ...
    def _update_html_urls(self, leewise_blocks, attachments_url_src, cropped_attachments_url_src):
        ...
        homepage = leewise_blocks['homepage']
        ...
        for page_name, page_dict in leewise_blocks.get('pages', {}).items():
            leewise_blocks['pages'][page_name]['body_html'] = update_page_html(page_dict)
