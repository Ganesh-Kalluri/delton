...
# This will be modified by a unique password on the first start of Leewise
...
wget 'https://nightly.leewise.in/master/iotbox/eftdvs' -P /usr/local/bin/
chmod +x /usr/local/bin/eftdvs
wget 'https://nightly.leewise.in/master/iotbox/eftapi.so' -P /usr/lib/

groupadd usbusers
usermod -a -G usbusers pi
usermod -a -G lp pi
usermod -a -G input lightdm
mkdir -v /var/log/leewise
chown pi:pi /var/log/leewise
chown pi:pi -R /home/pi/leewise/
...
echo "* * * * * rm /var/run/leewise/sessions/*" | crontab -
...
echo "addons/hw_drivers/iot_devices/" > /home/pi/leewise/.git/info/exclude
