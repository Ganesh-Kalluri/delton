...
from leewise import _, http, service
from leewise.tools.func import lazy_property
from leewise.tools.misc import file_path
...
class IoTRestart(Thread):
    """
    Thread to restart leewise server in IoT Box when we must return a answer before
    """
...
def check_certificate():
    ...
    if cn == 'LeewiseTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Leewise DB and
    ...
    try:
        ...
        if response.status == 200:
            git = ['git', '--work-tree=/home/pi/leewise/', '--git-dir=/home/pi/leewise/.git']
            ...
            _logger.info("Current IoT Box local git branch: %s / Associated Leewise database's git branch: %s", local_branch, db_branch)

            if db_branch != local_branch:
                with writable():
                    subprocess.check_call(["rm", "-rf", "/home/pi/leewise/addons/hw_drivers/iot_handlers/drivers/*"])
                    subprocess.check_call(["rm", "-rf", "/home/pi/leewise/addons/hw_drivers/iot_handlers/interfaces/*"])
                    subprocess.check_call(git + ['branch', '-m', db_branch])
                    subprocess.check_call(git + ['remote', 'set-branches', 'origin', db_branch])
                    os.system('/home/pi/leewise/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
...
def check_image():
    ...
    """
    url = 'https://nightly.leewise.in/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def save_conf_server(url, token, db_uuid, enterprise_code):
    ...
    """
    write_file('leewise-remote-server.conf', url)
    write_file('token', token)
    write_file('leewise-db-uuid.conf', db_uuid or '')
    write_file('leewise-enterprise-code.conf', enterprise_code or '')
...
def get_leewise_server_url():
    if platform.system() == 'Linux':
        ap = subprocess.call(['systemctl', 'is-active', '--quiet', 'hostapd']) # if service is active return 0 else inactive
        if not ap:
            return False
    return read_file_first_line('leewise-remote-server.conf')
...
def get_version():
    if platform.system() == 'Linux':
        return read_file_first_line('/var/leewise/iotbox_version')
...
def load_certificate():
    """
    Send a request to Leewise with customer db_uuid and enterprise_code to get a true certificate
    """
    db_uuid = read_file_first_line('leewise-db-uuid.conf')
    enterprise_code = read_file_first_line('leewise-enterprise-code.conf')
    if not (db_uuid and enterprise_code):
        return "ERR_IOT_HTTPS_LOAD_NO_CREDENTIAL"

    url = 'https://www.leewise.in/leewise-enterprise/iot/x509'
    data = {
        'params': {
            'db_uuid': db_uuid,
    ...
    try:
        ...
        _logger.exception("An error occurred while trying to reach leewise.in servers.")
    ...
    write_file('leewise-subject.conf', result['subject_cn'])
    if platform.system() == 'Linux':
        with writable():
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured Leewise server
    """
    server = get_leewise_server_url()
    if server:
        urllib3.disable_warnings()
        ...
        try:
            ...
            if resp.data:
                with writable():
                    drivers_path = ['leewise', 'addons', 'hw_drivers', 'iot_handlers']
...
def compute_iot_handlers_addon_name(handler_kind, handler_file_name):
    return "leewise.addons.hw_drivers.iot_handlers.{handler_kind}.{handler_name}".\
        format(handler_kind=handler_kind, handler_name=handler_file_name.removesuffix('.py'))

def load_iot_handlers():
    """
    This method loads local files: 'leewise/addons/hw_drivers/iot_handlers/drivers' and
    'leewise/addons/hw_drivers/iot_handlers/interfaces'
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\Leewise\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\Leewise\\downloaded_file.zip', 'C:\\Program Files\\Leewise\\new_folder'))
