...
QUnit.test("click a web link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Leewise](https://leewise.in)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    assert.strictEqual(urlRepresentation(cell.link, model.getters), "https://leewise.in");
    openLink(cell.link, env);
    assert.verifySteps(["https://leewise.in"]);
});
...
QUnit.test("click a menu link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[label](leewise://ir_menu_xml_id/test_menu)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
...
QUnit.test("click a menu link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: `[a view](leewise://view/${JSON.stringify(view)})` } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    assert.strictEqual(urlRepresentation(cell.link, model.getters), "an leewise view");
    openLink(cell.link, env);
