...
QUnit.test('create/search anytime appointment type', async function (assert) {
    ...
    patchWithCleanup(navigator, {
        clipboard: {
            writeText: (value) => {
                assert.strictEqual(
                    value,
                    `http://amazing.leewise.in/appointment/3?filter_staff_user_ids=%5B${uid}%5D`
                );
            }
        },
    });
    ...
    await makeView({
        ...
        session: {
            'web.base.url': 'http://amazing.leewise.in',
        },
    });
    ...
    patchWithCleanup(navigator, {
        clipboard: {
            writeText: (value) => {
                assert.strictEqual(
                    value,
                    `http://amazing.leewise.in/appointment/2?filter_staff_user_ids=%5B${uid}%5D`
                );
            }
        }
    });
