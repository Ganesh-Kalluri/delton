...
from leewise.tools import TEXT_URL_REGEX
...
class TestMailRenderMixin(common.HttpCase):
    ...
    def test_shorten_links_html_skip_shorts(self):
        ...
        new_content = self.env["mail.render.mixin"]._shorten_links(
            'Reusing this old <a href="{old_short_url}">link</a> with a new <a href="https://leewise.in">one</a>'
    ...
    def test_shorten_links_text_skip_shorts(self):
        ...
        new_content = self.env["mail.render.mixin"]._shorten_links_text(
            'Reusing this old link {old_short_url} with a new one, https://leewise.in</a>'
            .format(old_short_url=created_short_url),
            {}
        )
        expected = re.compile(
