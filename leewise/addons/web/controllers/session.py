...
import leewise
import leewise.modules.registry
from leewise import http
from leewise.modules import module
from leewise.exceptions import AccessError, UserError, AccessDenied
...
class Session(http.Controller):
    ...
    def get_session_info(self):
        # Crapy workaround for unupdatable Leewise Mobile App iOS (Thanks Apple :@)
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        if pre_uid != request.session.uid:
            # Crapy workaround for unupdatable Leewise Mobile App iOS (Thanks Apple :@) and Android
        ...
        registry = leewise.modules.registry.Registry(db)
        with registry.cursor() as cr:
            env = leewise.api.Environment(cr, request.session.uid, request.session.context)
    ...
    def account(self):
        ...
        return 'https://www.leewise.in/oauth2/auth?' + url_encode(params)
