...
class TestCommissionsSetup(AccountTestInvoicingCommon):
    ...
    def _make_products(self):
        ...
        # leewise sh
        self.leewise_sh = self.env['product.category'].create({
            'name': 'Leewise.SH',
        })

        self.worker = self.env['product.product'].create({
            'name': 'Leewise.sh Worker',
            'categ_id': self.leewise_sh.id,
            'list_price': 100.0,
        ...
        self.staging = self.env['product.product'].create({
            'name': 'Leewise.sh Staging Branch',
            'categ_id': self.leewise_sh.id,
            'list_price': 30.0,
            'recurring_invoice': True,
            'purchase_ok': True,
    ...
    def _make_commission_plans(self):
        self.learning_plan = self.env['commission.plan'].create({
            ...
            'commission_rule_ids': [
                (0, 0, self._make_rule(self.leewise_sh, 10, product=self.worker, pricelist=self.eur_20, is_capped=True, max_comm=150)),
                (0, 0, self._make_rule(self.leewise_sh, 10, product=self.worker, pricelist=self.usd_8, is_capped=True, max_comm=180)),
        ...
        self.ready_plan = self.env['commission.plan'].create({
            ...
            'product_id': self.env.ref('partner_commission.product_commission').id,
            'commission_rule_ids': [
                (0, 0, self._make_rule(self.leewise_sh, 100, product=self.worker, pricelist=self.eur_20, is_capped=True, max_comm=150)),
                (0, 0, self._make_rule(self.leewise_sh, 100, product=self.worker, pricelist=self.usd_8, is_capped=True, max_comm=180)),
                (0, 0, self._make_rule(self.apps_support, 10)),
            ],
        })
        ...
        self.gold_plan = self.env['commission.plan'].create({
            ...
            'product_id': self.env.ref('partner_commission.product_commission').id,
            'commission_rule_ids': [
                (0, 0, self._make_rule(self.leewise_sh, 100, pricelist=self.eur_20, is_capped=True, max_comm=150)),
                (0, 0, self._make_rule(self.leewise_sh, 100, pricelist=self.usd_8, is_capped=True, max_comm=180)),
        ...
        self.greedy_plan = self.env['commission.plan'].create({
            ...
            'product_id': self.env.ref('partner_commission.product_commission').id,
            'commission_rule_ids': [
                (0, 0, self._make_rule(self.leewise_sh, 100, pricelist=self.usd_8, is_capped=True, max_comm=18)),
            ],
        })
