...
from leewise.tools import format_date, get_timedelta
from leewise.tests.common import Form, tagged
from leewise.addons.partner_commission.tests.setup import Line, Spec, TestCommissionsSetup
from leewise.tools.misc import NON_BREAKING_SPACE
...
class TestPurchaseOrder(TestCommissionsSetup):
    def test_automatic_confirm(self):
        ...
        # Case: OK.
        with patch('leewise.fields.Date.today', today):
            with patch('leewise.addons.mail.models.mail_template.MailTemplate.send_mail', _patched_send_mail):
    ...
    def test_vendor_bill_description_multi_line_format(self):
        ...
        expected = f"""Commission on INV/12345/0001, Customer, 2,000.00{NON_BREAKING_SPACE}€\n{so.name}, \nLeewise.sh Worker: from {format_date(self.env, date_from)} to {format_date(self.env, date_to)} (12 month(s))"""
