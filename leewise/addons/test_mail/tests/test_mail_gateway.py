...
from leewise import exceptions
from leewise.addons.mail.models.mail_thread import MailThread
from leewise.addons.mail.tests.common import mail_new_test_user, MailCommon
from leewise.addons.test_mail.data import test_mail_data
from leewise.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE
from leewise.addons.test_mail.models.test_mail_models import MailTestGateway
from leewise.sql_db import Cursor
from leewise.tests import tagged, RecordCapturer
from leewise.tools import email_split_and_format, formataddr, mute_logger
...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs('leewise.addons.mail.models.mail_thread', level="WARNING") as capture:
        ...
        self.assertEqual(capture.output, [
            ("WARNING:leewise.addons.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        incoming_bounce = self.format(
            ...
            email_from='MAILER-DAEMON@mailserver.leewise.in (Mail Delivery System)',
            email_to='bounce@xxx.leewise.in',
            delivered_to='bounce@xxx.leewise.in'
        )
...
class TestMailgateway(MailCommon):
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread', 'leewise.models')
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not leewisebot:
        ...
        leewisebot = self.env.ref('base.partner_root')
        leewisebot.active = True
        leewisebot.email = 'leewisebot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, leewisebot.email_formatted, f'groups@{self.alias_domain}',
                subject='Leewisebot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, leewisebot)
        self.assertEqual(record4.message_ids[0].email_from, leewisebot.email_formatted)
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: leewisebot -> no follower')
    ...
    @mute_logger('leewise.addons.mail.models.mail_mail', 'leewise.addons.mail.models.mail_thread', 'leewise.models.unlink', 'leewise.tests')
    ...
    @mute_logger('leewise.addons.mail.models.mail_mail', 'leewise.addons.mail.models.mail_thread', 'leewise.models')
    def test_message_route_alias_owner_author_notify(self):
        ...
        self.assertFalse(self.user_root.active, 'notification logic relies on leewisebot being archived')
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'Leewisebot should be creating the message')
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread', 'leewise.models.unlink', 'leewise.addons.mail.models.mail_mail')
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread', 'leewise.addons.mail.models.mail_mail', 'leewise.models.unlink')
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread', 'leewise.addons.mail.models.mail_mail', 'leewise.models', 'leewise.sql_db')
    def test_message_process_alias_config_invalid_defaults(self):
        ...
        with self.mock_mail_gateway(), patch('leewise.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
        ...
        with self.assertRaises(Exception), patch('leewise.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
                                                 autospec=True) as _alias_bounce_incoming_email_mock:
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread', 'leewise.models', 'leewise.tests')
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread', 'leewise.models.unlink')
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread', 'leewise.models.unlink', 'leewise.addons.mail.models.mail_mail', 'leewise.tests')
    def test_message_process_external_notification_reply(self):
        ...
        self.assertFalse(new_msg.is_internal, "Responses to messages sent by leewisebot should always be public.")
        ...
        new_msg = self.test_record.message_ids[0]
        self.assertTrue(new_msg.is_internal, "Responses to messages sent by anyone but leewisebot should keep"
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by leewise should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        leewise should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        ...
        # Leewise 10 update: compat mode has been removed and should not work anymore
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Leewise is still capable of decoding it.
    ...
    @mute_logger('leewise.addons.mail.models.mail_thread', 'leewise.addons.mail.models.mail_mail')
    ...
    def test_routing_loop_alias(self):
        ...
        with self.mock_mail_gateway():
            self.format_and_process(
                ...
                extra='References: <test-1337-loop-detection-bounce-email@leewise.in>',
            )
