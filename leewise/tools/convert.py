...
from leewise import SUPERUSER_ID, api
...
def _get_idref(self, env, model_str, idref):
    idref2 = dict(idref,
                  Command=leewise.fields.Command,
                  time=time,
                  DateTime=datetime,
                  datetime=datetime,
                  timedelta=timedelta,
                  relativedelta=relativedelta,
                  version=leewise.release.major_version,
                  ref=self.id_get,
                  pytz=pytz)
    if model_str:
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval:
            ...
            try:
                ...
                logging.getLogger('leewise.tools.convert.init').error(
        ...
        # invoke method
        return leewise.api.call_kw(model, method_name, args, kwargs)
    elif node.tag == "test":
        return node.text
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        for group in rec.get('groups', '').split(','):
            if group.startswith('-'):
                ...
                groups.append(leewise.inmand.unlink(group_id))
            elif group:
                group_id = self.id_get(group)
                groups.append(leewise.inmand.link(group_id))
        if groups:
    ...
    def _tag_record(self, rec, extra_vals=None):
        ...
        for field in rec.findall('./field'):
            ...
            if f_search:
                ...
                if (f_name in _fields) and _fields[f_name].type == 'many2many':
                    f_val = [leewise.inmand.set([x[f_use] for x in s])]
                elif len(s):
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler leewise.tools.convert:DEBUG" for complete traceback
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <openerp>, <leewise> or <data>."
        self._tag_root(de)
    DATA_ROOTS = ['leewise', 'data', 'openerp']
